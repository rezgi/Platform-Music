...
# Where do (Apostrophe) in play and () in stop tooltips come from ?

onready var i := preload("res://Metronome/Scripts/metronome_inputs.gd").new()
onready var icon_pause := preload("res://Metronome/Assets/Icons/icon_pause.tres")
...
enum btn_status {ACTIVE, DISABLED, ENTERED, EXITED}
...
var metronome_clics := {measure = false, beat = false, sixteen = false}
...
func _on_TextEditBPM_text_changed() -> void:
	...
	if not bpm:
		input_is_ok = false
		i.disable_play(btn_play, warning_play)
	else:
		input_is_ok = true
...
func _on_ButtonStop_pressed() -> void:
	...
	else: btn_stop.modulate = Color("32ffffff")

func _on_ButtonStop_mouse_entered() -> void:
	btn_stop.modulate = i.check_color(metronome_status.play, "entered")

func _on_ButtonStop_mouse_exited() -> void:
	btn_stop.modulate = i.check_color(metronome_status.play, "exited")
...
func _on_ButtonPlay_toggled(button_pressed: bool) -> void:
	...
	if not button_pressed:
		# means that it's either paused or stopped
		pass
	if not input_is_ok:
...
func _on_ButtonPlay_mouse_entered() -> void:
	btn_play.modulate = i.check_color(input_is_ok, "entered")

func _on_ButtonPlay_mouse_exited() -> void:
	btn_play.modulate = i.check_color(input_is_ok, "exited")
